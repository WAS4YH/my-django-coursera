...
def submit(request, course_id):
    ...
    submission = Submission.objects.create(enrollement=enrollment, mode='honor')
...
def show_exam_result(request, course_id, submission_id):
    ...
    total_score = 0
    question = course.question_set.all()

    for question in questions:
        correct_choices = question.choice_set.filter(is_correct=True)  # Get all correct choices for the question
        selected_choices = choices.filter(content=question)  # Get the user's selected choices for the question
